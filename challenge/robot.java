...
public class robot{
    public static void main(String []sdf){
        ...
        int posX=s.nextInt();
        int posY=s.nextInt();s.nextLine();
        String sss=s.nextLine(),ssL=s.nextLine();
        String []ll=sss.split("-");
        String []loc=ssL.split(" ");
        int rowX=Integer.parseInt(ll[0]),colY=Integer.parseInt(ll[1]);
        String dir=ll[2];
        String er="";
        String dir1="WNESWN";
        ...
        {
            String ss=loc[i];
                if(ss.equals("L") || ss.equals("R"))
                 {
                    if(ss.equals("L")){
                        for(int j=dir1.length()-1;j>=0;j--)
                        {
                            if(dir.equals(Character.toString(dir1.charAt(j)))){
                                dir=Character.toString(dir1.charAt(--j));
                                break;
                            }
                        }
                    } else {
        ...
        System.out.print(rowX+"-"+colY+"-"+dir+er);
    }
}
